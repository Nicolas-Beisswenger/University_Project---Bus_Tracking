...
public class Bus_01 extends Thread{
    ...
    public void run() {
        ...
        while(activo == true){
            // Condición para arrancar a partir de las 5 am
            if (Tiempo.Horas_Contador >= 5){
                
                //Condición para determinar si el bus está destacado o no
                if (label_bus_destacado == false){
                    Interfaz_Grafica.label_bus01.setIcon(new ImageIcon(Interfaz_Grafica.imgbus1));
                    Interfaz_Grafica.label_bus01.setLocation(locacion_X , locacion_Y);
                }
                else if (label_bus_destacado == true){
                    Interfaz_Grafica.label_bus01.setIcon(new ImageIcon(Interfaz_Grafica.imgbus1G));
                    Interfaz_Grafica.label_bus01.setLocation(locacion_X , locacion_Y);
                }
                
                //Velocidad a la que el bus cambiara de posición
                try {
                ...
                // Identificador para determinar hacia que dirección moverse
                ...
                //Estaciones
                //Determinar si el bus se encuentra o no en una parada
                if (locacion_X == acceso_infoestaciones.X_CiudadDeportiva && locacion_Y == acceso_infoestaciones.Y_CiudadDeportiva){
                    
//Determinar si el bus debe o no saltarse la estación
                    if (evitar_estacion == false){
                        
                        //Declaración de en que estación se encuentra
                        num_estacion = 1;
                            nom_estacion = "Ciudad Deportiva Rafael Angel Perez";
                                
                            //Identificador si el bus de adelante se enccuentra en la misma estación
                            if (Bus_10.num_estacion == num_estacion && Bus_10.num_estacion > 0 && num_estacion > 0){ 
                                    Bus_10.evitar_estacion = true;
                                }
                                ...
                                try {
                                    TimeUnit.MILLISECONDS.sleep(2000);
                                        ...
                                        //Declaración de salida de la estación
                                        num_estacion = 0;
